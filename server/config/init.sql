...
CREATE TABLE Recipes(id integer unsigned, name varchar(320), creator varchar(320), rating tinyint(1) unsigned, content text, prepTime integer unsigned, totalTime integer unsigned, PRIMARY KEY(id), FOREIGN KEY(creator) REFERENCES users(email) ON DELETE SET NULL ON UPDATE CASCADE);
...
FOR EACH ROW UPDATE recipes
	SET rating = (SELECT AVG(rating) FROM votes V WHERE V.recipe = new.recipe)
WHERE id = NEW.recipe;
ALTER TABLE `glo-2005`.`recipes` DROP PRIMARY KEY, ADD PRIMARY KEY (`id`) USING BTREE;
ALTER TABLE `glo-2005`.`recipeingredients` DROP INDEX `recipe`, ADD INDEX `recipe` (`recipe`) USING BTREE;
ALTER TABLE `glo-2005`.`recipeingredients` DROP INDEX `ingredient`, ADD INDEX `ingredient` (`ingredient`) USING BTREE;
ALTER TABLE `glo-2005`.`comments` DROP INDEX `recipe`, ADD INDEX `recipe` (`recipe`) USING BTREE;